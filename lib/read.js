...
function readFile(readChar, cb) {
    ...
    readCommChars(readChar, function readFileChar(ch, cb) {
        if (ch === ' ') {
            ...
            buffer.next();
            readRuleBody(readChar, function (err, prereqs, commands, leftover) {
                if (err) return cb(err);
...
function readRuleBody(readChar, cb) {
    var buffer = new ListBuffer();
    readCommChars(readChar, function (ch, cb) {
...
function readRuleCommand(readChar, cb) {
    var command = '';
    readEscChars(readChar, function (ch, cb) {
        if (ch === '\n') return cb(false);
        if (command.length === 0 && (ch === ' ' || ch === '\t')) return cb();
        command += ch;
    }, function (err) {
        return cb(err, command);
    });
}
